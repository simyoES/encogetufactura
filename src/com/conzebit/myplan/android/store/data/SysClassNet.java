...
public final class SysClassNet {
	...
	private static final String LOG_TAG = "SysClassNet";
	...
	public static boolean isAvail(final String inter) {
		try {
			...
			Log.i(LOG_TAG, "could not read device: " + inter);
		}
		return false;
	}
	...
	private static long readLong(final String inter, final String file) {
		...
		try {
			...
			Log.e(LOG_TAG, e.getMessage() + " / error readding long for inter: " + inter);
			return 0;
		} finally {
			if (raf != null) {
				try {
